...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            e (float): Expansion ratio.
            cbam (bool): Whether to inject CBAM in residual branch.
        """
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = Conv(c_, c2, k[1], 1, g=g)
        self.attn = CBAM(c2) if cbam else nn.Identity()
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        attn = getattr(self, "attn", nn.Identity())
        y = attn(self.cv2(self.cv1(x)))
...
class C3k2CBAM(C3k2):
    """# 优化：在主干残差块内部使用带残差的 CBAM 瓶颈，不破坏原有 C3k2 外部结构。"""
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            nn.Sequential(
                Bottleneck(self.c, self.c, shortcut, g, cbam=True),
                PSABlock(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1)),
            )
            if attn
            else C3k(self.c, self.c, 2, shortcut, g)
            if c3k
            else Bottleneck(self.c, self.c, shortcut, g, cbam=True)
            for _ in range(n)
        )
...
class C3k2CBAM(nn.Module):
    '''C3k2 block followed by CBAM attention (drop-in replacement for C3k2).

    YAML usage: replace `C3k2` with `C3k2CBAM` (same args).
    '''
    ...
    def __init__(
        ...
        super().__init__()
        self.m = C3k2(c1, c2, n=n, c3k=c3k, e=e, attn=attn, g=g, shortcut=shortcut)
        self.cbam = CBAM(c2, kernel_size=cbam_kernel)

    def forward(self, x):
        return self.cbam(self.m(x))
